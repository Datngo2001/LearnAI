...
class Plan:
    ...
    def __init__(self, action=None, conditions=[]):
        self.action = action
...
def isOnPath(state, path):
    if state in path: return True
    else: return False


def Or_Search(state, problem, path):
    if problem.goal_test(state): return []
    if isOnPath(state, path): return failure
    actions = problem.actions(state)
    for action in actions:
        path.insert(0, state)
        actionResult = problem.result(state, action)
        plan = And_Search(actionResult, problem, path)
        if plan != failure:
            plan.action = action
            return plan
    return failure
...
def And_Search(states, problem, path):
    plan = Plan()
    for state in states:
        plani = Or_Search(state, problem, path)
        if plani == failure: return failure
        plan.conditions.append((state, plani))
    return plan


def printRs(plan: Plan):
    if plan.action == None or plan.conditions == []: return "Goal!!!"
    rs = "Action: " + str(plan.action) + "\n"
    for condition in plan.conditions:
        rs += ("(" + str(condition[0]) + ", " + str(condition[1]) + ")\n")
    return rs
